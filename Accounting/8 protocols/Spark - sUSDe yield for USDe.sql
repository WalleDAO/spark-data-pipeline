...
with seq as (
    ...
    from unnest(
        ...
        cross join (
            select 'USDe' as symbol,
                0 as amount
            union all
            select 'USDe_pay' as symbol,
                0 as amount
            union all
            select 'sUSDe' as symbol,
                0 as amount
            union all
            select 'u-USDe' as symbol,
                0 as amount
        ) t
),
balance_sll as (
    ...
    select date(evt_block_time) as dt,
        ...
        sum(
            case
                when "to" = 0x1601843c5E9bC251A3272907010AFa41Fa18347E then "value"
                when "from" = 0x1601843c5E9bC251A3272907010AFa41Fa18347E then - "value"
                else 0
            end
        ) / 1e18 as amount
    ...
    from ethena_labs_ethereum.usde_evt_transfer
    where "to" = 0x1601843c5E9bC251A3272907010AFa41Fa18347E -- SSL
        and "from" not in (
    ...
    from (
            ...
            select date(evt_block_time) as dt,
                - sum(value) / 1e18 as amount
            from ethena_labs_ethereum.usde_evt_transfer
            where "from" = 0x7FC7c91D556B400AFa565013E3F32055a0713425 -- Silo
...
balance_ssl_cum as (
    ...
    from seq s
        left join balance_sll b using (dt, symbol)
),
...
susde_price_usd as (
    ...
    from share_pricing
    where token_address = 0x9D39A5DE30e57443BfF2A8307A4256c8797A3497 -- sUSDe
),
balance_ssl_final as (
    ...
    from balance_ssl_pivot_clean b
        left join susde_price_usd p using (dt)
),
