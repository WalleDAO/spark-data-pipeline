...
transaction_days_avg as (
    select
        ...
        case
            when sum(date_diff('second', ts, end_time)) > 0
                then sum(
                running_balance * date_diff('second', ts, end_time)
...
complete_daily_balances as (
    select
        ...
        c.dt,
        case
            when t.time_weighted_avg_balance is not null
                then t.time_weighted_avg_balance -- Transaction day: use time-weighted average
            else -- Non-transaction day: use previous transaction day's end balance
