----https://dune.com/queries/5595724
with
    -- Farm addresses
    ...
    -- Extract raw transfer data for staking farms
    raw_transfers as (
        select 
            ...
            if(t."to" = f.contract_address, t."value", -t."value") / 1e18 as amount_change
        from sky_ethereum.usds_evt_transfer t
        join farm_addr f on f.contract_address in (t."from", t."to")
        where if(t."to" = f.contract_address, t."value", -t."value") != 0 -- Only process records with balance changes
    ),
